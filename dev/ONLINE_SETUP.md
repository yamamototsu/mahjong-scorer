...
**貼り替えるときは `dev/firebase-rules.json` を丸ごとコピーするのが確実。**
下の2つ（既存ぶん・クラウド保存ぶん）を合わせたものが、そのファイルに入っている。
...
1. 両方のスマホでアプリを開き「👥 友達」→ それぞれ名前を登録（個人IDが発行される）
2. 追加のしかたは3つ。どれか片方がやれば、お互いのリストに載る
   - 「📷 QRコードを読み取る」で相手の画面のQRを読む
   - 「📤 追加用のリンクを送る」で送ったリンクを、相手が開く
   - 相手の6文字の個人IDを入力して「追加」
3. 追加した友達の名前は、この端末の名簿にも入る（呼び名は ✏️ で変えられる。
   個人IDに結びつけて持つので、名前がかぶっても取り違えない）
4. その友達をメンバーに入れて対局し「✓ 終了」→ 相手の受信箱へ自動で送られる
5. 受け取る側は「👥 友達」か「📋 対局履歴」を開くだけで履歴に入る
   → 通算成績にも反映される。同じ対局を二重に受け取っても重複しない
6. 手で送りたいときは、履歴 →「📤 友達に送る」→ 対局を選ぶ → 送り先を選ぶ
   （友達でない人から届いたものは、受信箱に残るので「取り込む」を押す）

※ カメラでの読み取りは Android などは端末の機能を使い、iPhone では jsQR
（cdnjs / jsDelivr から読み込む）を使う。どちらも使えないときは「写真から
読み取る」か、個人IDの手入力になる。
